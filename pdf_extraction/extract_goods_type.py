...
def extract_goods_type(text):
    ...
    for i in range(len(lines) - 1):  # Prevent index error
        line = lines[i]
        match = re.search(r'\d*\s*Cartons\s+(.*?)\s+P', line)  # Allow numbers attached to "Cartons"
        
        if match:
            goods_type = match.group(1).strip()  # Trim any extra spaces
            
            # Check the next line for additional information
            next_line = lines[i + 1].strip()
            if len(next_line.split()) <= 3 and not next_line.split()[-1].isdigit():
                goods_type += " " + next_line  # Append the next line
